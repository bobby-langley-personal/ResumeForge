...
export default function Home() {
  ...
  // Pre-generation refs — background generation starts 3s after fit analysis
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Schedule background generation — starts 3s after user sees fit analysis
  ...
  // Runs in background after a delay — writes to refs, never touches state
  const runPreGeneration = async (formData: FormData, analysis: FitAnalysis) => {
  ...
  const handleGenerateDocuments = async () => {
    ...
    // Cancel any pending pre-gen timer
  ...
  const resetForm = () => {
    ...
    if (preGenStatus.current === 'pending' || preGenStatus.current === 'running') {
      ...
      // Log the abandon — visible in Vercel function logs
